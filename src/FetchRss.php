 ...
 * File:        FetchRss.php, a simple functional interface
 ...
 * feel to Magpie, see RssCache.php for rationale
 *
 *
...
/*=======================================================================*\
    ...
    Output:   parsed RSS object (see MagpieRss.php)
...
class FetchRss{
     function parse($url) {
        ...
        if ( !MAGPIE_CACHE_ON ) {
            ...
            if ( $this->is_success( $resp->getStatusCode() ) ) {
        ...
        // else cache is ON
            ...
            if (isset($resp) and $resp) {
                ...
                elseif ( $this->is_success( $resp->getStatusCode() ) ) {
                    ...
                    if ( $resp->getStatusCode() == '-100' ) {
                        ...
                        $errormsg .=  "(HTTP Response: " . $resp->getStatusCode() .')';
                    }
                }
            }
            else {
    ...
    function _fetch_remote_file ($url, $headers = [] ) {
        ...
        $client = new \GuzzleHttp\Client([
            'headers'   => array_merge(
                $headers,
                [
                    'User-Agent' =>   MAGPIE_USER_AGENT,
                    'timeout' => MAGPIE_FETCH_TIME_OUT,
                    'decode_content' => MAGPIE_USE_GZIP,
    ...
    \*=======================================================================*/
    function _response_to_rss (ResponseInterface $resp) {
        ...
        if ( $rss and !$rss->ERROR) {
            ...
            foreach($resp->getHeaders() as $field => $values) {
